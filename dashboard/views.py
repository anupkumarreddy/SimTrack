...
class DashboardView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        project_rows = []
        for project in Project.objects.select_related('owner').annotate(
            ...
            completed_milestone_count=Count('milestones', filter=Q(milestones__status=MilestoneStatus.COMPLETED), distinct=True),
        ).order_by('name'):
        ...
        ctx['top_failing_regressions'] = sorted(regression_rows, key=lambda row: row['fail_rate_value'], reverse=True)[:5]
        ctx['top_passing_regressions'] = sorted(regression_rows, key=lambda row: row['pass_rate_value'], reverse=True)[:5]
        ...
        ctx['top_signatures'] = (
            FailureSignature.objects
            .select_related('regression_run__regression__project')
