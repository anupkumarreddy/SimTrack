...
class ProjectForm(forms.ModelForm):
    new_category_name = forms.CharField(
        ...
        required=False,
        max_length=100,
        widget=forms.TextInput(attrs={'class': 'block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm'}),
    )

    class Meta:
        model = Project
        fields = ['name', 'description', 'category', 'new_category_name', 'owner', 'status', 'repository_url', 'is_active']
        widgets = {
            ...
            'category': forms.Select(attrs={'class': 'block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm'}),
