...
class UserManagerCreateUserTests(TestCase):
    ...
    def test_email_is_normalized(self):
        user = User.objects.create_user(
            email="Test@Example.COM", username="normalized", password="testpass"
        )
        self.assertEqual(user.email, "Test@example.com")

    def test_password_is_hashed_not_plaintext(self):
        user = User.objects.create_user(
            email="pw@example.com", username="hashed", password="testpass"
        )
    ...
    def test_password_none_is_allowed(self):
        user = User.objects.create_user(
            email="nopw@example.com", username="nopassword", password=None
        )
        self.assertIsNotNone(user.pk)
    ...
    def test_default_is_staff_is_false(self):
        user = User.objects.create_user(
            email="staff@example.com", username="staffcheck", password="testpass"
        )
        self.assertFalse(user.is_staff)

    def test_default_is_active_is_true(self):
        user = User.objects.create_user(
            email="active@example.com", username="activecheck", password="testpass"
        )
        self.assertTrue(user.is_active)

    def test_default_is_superuser_is_false(self):
        user = User.objects.create_user(
            email="super@example.com", username="supercheck", password="testpass"
        )
    ...
    def test_username_must_be_unique(self):
        User.objects.create_user(
            email="a@example.com", username="dupe", password="testpass"
        )
        with self.assertRaises(Exception):
            User.objects.create_user(
                email="b@example.com", username="dupe", password="testpass"
            )

    def test_email_must_be_unique(self):
        User.objects.create_user(
            email="dupe@example.com", username="user1", password="testpass"
        )
        with self.assertRaises(Exception):
            User.objects.create_user(
                email="dupe@example.com", username="user2", password="testpass"
            )

    def test_dunder_str_returns_username(self):
        user = User.objects.create_user(
            email="str@example.com", username="struser", password="testpass"
        )
...
class UserManagerCreateSuperuserTests(TestCase):
    ...
    def test_email_is_normalized(self):
        user = User.objects.create_superuser(
            email="Admin@Example.COM", username="admin2", password="adminpass"
        )
        self.assertEqual(user.email, "Admin@example.com")

    def test_password_is_hashed(self):
        user = User.objects.create_superuser(
            email="hash@example.com", username="hashadmin", password="adminpass"
        )
    ...
    def test_username_must_be_unique(self):
        User.objects.create_superuser(
            email="a1@example.com", username="superdupe", password="adminpass"
        )
        with self.assertRaises(Exception):
            User.objects.create_superuser(
                email="a2@example.com", username="superdupe", password="adminpass"
            )

    def test_email_must_be_unique(self):
        User.objects.create_superuser(
            email="dupe@example.com", username="sd1", password="adminpass"
        )
        with self.assertRaises(Exception):
            User.objects.create_superuser(
                email="dupe@example.com", username="sd2", password="adminpass"
            )

    def test_dunder_str_returns_username(self):
        user = User.objects.create_superuser(
            email="str@example.com", username="strsuper", password="adminpass"
        )
        self.assertEqual(str(user), "strsuper")

    def test_is_active_defaults_true(self):
        user = User.objects.create_superuser(
            email="active@example.com", username="activesuper", password="adminpass"
        )
        self.assertTrue(user.is_active)
